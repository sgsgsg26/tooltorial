...
### 1.2 MVP 범위표
* **핵심 행동 1문장:** 사용자가 사원증(RFID)을 태그하여 공구를 선택하면, 자판기가 공구를 자동 배출하고 AI가 실시간으로 공구의 파손 상태를 판별하여 화면에 피드백을 제공한다.
* **이번에 만든 것 (MVP 포함):** 
...
* **이번에 안 만든 것 (MVP 제외):**
  - <s>DB 연동 회원가입 및 소셜 로그인 (오프라인 RFID 인증으로 속도 확보)</s>
  - <s>재고 파악 및 발주를 위한 관리자 전용 원격 대시보드</s>
  - <s>공구 반납 시스템 및 연체 추적 기능</s>
...
## 9. 🔒 보안 점검 완료
- **비밀키 노출 점검:** 클라우드(OpenAI, AWS 등)를 거치지 않는 오프라인 로컬 구동(Localhost, Web Serial) 환경이므로, 소스 코드 전체를 스크리닝한 결과 과금이 발생할 수 있는 외부 API Key나 DB 비밀번호가 하드코딩되어 있지 않음을 100% 확인했습니다.
